 ...
 */
public class SessionIDAdapter extends InnAdapter {
    ...
    public Object getValueFromPS(ProfileStreamMessage profileStreamMessage, FieldsEntry fieldsEntry) throws MappingDataException {
        return convertValue(profileStreamHelper.getSession(profileStreamMessage).getId(), fieldsEntry);
    }
    ...
    public void setValueToProfile(Profile profile, FieldsEntry fieldsEntry, Map<String, Object> map) throws MappingDataException {
        ...
        if (val instanceof String){
            profile.getSessions().get(0).setId((String) val);
        }else {
            throw new MappingDataException("Session id in field ["+ fieldsEntry.getFieldName()+"] must be STRING convert type");
        }
