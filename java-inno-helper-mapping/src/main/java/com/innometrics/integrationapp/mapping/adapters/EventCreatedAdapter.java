...
public class EventCreatedAdapter extends InnAdapter {
    ...
    public void setValueToProfile(Profile profile, FieldsEntry fieldsEntry, Map<String, Object> map) throws MappingDataException {
       ...
       if (date instanceof Date) {
           profile.getSessions().get(0).getEvents().get(0).setCreatedAt((Date) date);
       }else {
           throw new MappingDataException("Event createrdAt in field "+ fieldsEntry.getFieldName()+"must be DATE convert type");
       }
    }
