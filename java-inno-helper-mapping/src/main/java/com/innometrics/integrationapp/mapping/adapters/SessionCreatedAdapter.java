 ...
 */
public class SessionCreatedAdapter extends InnAdapter {
    ...
    public Object getValueFromPS(ProfileStreamMessage profileStreamMessage, FieldsEntry fieldsEntry) throws MappingDataException {
        return convertValue(profileStreamHelper.getSession(profileStreamMessage).getCreatedAt(), fieldsEntry);
    }
    ...
    public void setValueToProfile(Profile profile, FieldsEntry fieldsEntry, Map<String, Object> map) throws MappingDataException {
        ...
        if (date instanceof Date) {
            profile.getSessions().get(0).setCreatedAt((Date) date);
        }else {
            throw new MappingDataException("Session createrdAt in field "+ fieldsEntry.getFieldName()+"must be DATE convert type");
        }
    }
}
