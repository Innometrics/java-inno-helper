 ...
 */
public class ProfileCloud implements Serializable {
    private static final Logger logger = Logger.getLogger(ProfileCloud.class.getCanonicalName());
    ...
    public ProfileCloud(String host, int port, int cacheSize, int cacheTTL) throws MalformedURLException {
        this(host, port, null, cacheSize, cacheTTL);
    }

    public ProfileCloud(String host) throws MalformedURLException {
        this(host, DEFAULT_PORT, DEFAULT_SIZE, DEFAULT_TTL);
    }

    public ProfileCloud(String host, int port) throws MalformedURLException {
        this(host, port, DEFAULT_SIZE, DEFAULT_TTL);
    }

    public ProfileCloud(String host, int port, AuthMethod auth) throws MalformedURLException {
        this(host, port, auth, DEFAULT_SIZE, DEFAULT_TTL);
    }


    public ProfileCloud(String host, int port, AuthMethod auth, int cacheSize, int cacheTTL) throws MalformedURLException {
...
//todo разобраться что за авторизация
    public ProfileCloud withAuth(AuthMethod auth) {
        if (auth != null) {
    ...
    public ProfileCloud withHeader(String header, String value) {
        this.headers.put(header, value);
        return this;
    }

    public ProfileCloud withParameter(ProfileCloudOptions key, String value) {
    ...
    public FutureTask<Pair<Integer, String>> deleteProfile(String companyId, String bucketId, String profileId) throws ExecutionException {
        return deleteProfile(companyId, bucketId, profileId, null, null);
    }

    public FutureTask<Pair<Integer, String>> deleteProfile(String companyId, String bucketId, String profileId, Map<ProfileCloudOptions, String> tmpParams, Map<String, String> tmpHeaders) throws ExecutionException {
        return deleteObject(new RestURI(hostWithVersion)
                .withResource(companies, companyId)
                .withResource(buckets, bucketId)
                .withResource(profiles, profileId), tmpParams, tmpHeaders);
    }
...
//            Profile toEvaluate = getProfile(companyId, bucketId, profileId, tmpParams, tmpHeaders).getProfile();
//            IqlResult[] toReturn = new IqlResult[segments.length];
//            for (int i = 0; i < segments.length; i++) {
//                toReturn[i] = evaluateProfile(toEvaluate, segments[i].getSegment(), doFiltering);
//            }
//            return toReturn;
//        } else {
...
//    }
//
//    public IqlResult[] evaluateProfile(String companyId, String bucketId, String profileId, boolean doFiltering) throws InterruptedException, ExecutionException,  IqlSyntaxException {
//        return evaluateProfile(companyId, bucketId, profileId, doFiltering, null, null);
//    }

    // todo переделать в зависимости от сегментов
//    public IqlResult evaluateProfile(String companyId, String bucketId, String profileId, String segmentId, boolean doFiltering, Map<ProfileCloudOptions, String> tmpParams, Map<String, String> tmpHeaders) throws InterruptedException, ExecutionException, JsonProcessingException, IqlSyntaxException {
//        return evaluateProfile(getProfile(companyId, bucketId, profileId, tmpParams, tmpHeaders).getProfile(), getSegment(companyId, bucketId, segmentId, tmpParams, tmpHeaders).getSegment(), doFiltering);
//    }
//
//    public IqlResult evaluateProfile(String companyId, String bucketId, String profileId, String segmentId, boolean doFiltering) throws InterruptedException, ExecutionException, JsonProcessingException, IqlSyntaxException {
//        return evaluateProfile(getProfile(companyId, bucketId, profileId).getProfile(), getSegment(companyId, bucketId, segmentId).getSegment(), doFiltering);
//    }
    ...
    private FutureTask<Pair<Integer, String>> deleteObject(RestURI url, Map<ProfileCloudOptions, String> tmpParams, Map<String, String> tmpHeaders) throws ExecutionException {
        URL endpoint = buildWithParams(url, tmpParams);
        httpObjectCache.clearByKey(endpoint.toString());
        return process(endpoint, HttpMethods.DELETE, String.class, tmpHeaders);
    }
